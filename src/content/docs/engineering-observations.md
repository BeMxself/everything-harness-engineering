...
这里主要关注的是 `harness engineering` 的工程观察：人如何组织长期开发、交接、控制与评审。它们当然会反过来影响 `agent harness` 的设计，但不应被直接误读成判断 `agent harness` 的唯一标准。
...
- 比较 `agent harness` 时，不能只比较“会不会改代码”，还要比较 `谁在控制下一步`
...
- 好的 `harness engineering` 实践，不只是任务能暂停和继续，而是要求 `agent harness` 在继续时减少重新理解成本
...
## 模型供应现实会同时塑造 harness 与 harness engineering

很多 `agent harness` 设计与 `harness engineering` 实践并不是固定教条，而是在模型能力、供应稳定性、成本、上下文窗口和工具可用性变化下不断漂移。模型升级会让一部分脚手架失去必要性，也会暴露新的瓶颈。
