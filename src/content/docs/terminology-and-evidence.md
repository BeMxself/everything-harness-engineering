...
| `官方表述` | 项目官方对自己的定位、自述和命名方式 | “X 官方称自己是编程代理 / agent harness / 工作流系统” |
| `研究解释` | 为了让比较成立而做出的工程划分与解释 | “文中把它放到 `agent harness` / `harness engineering` 实践层 / 工作流编排里” |
...
- 某个项目更适合被看作“以宿主为中心”
- 某套系统更像“叠加式工作流层”，而不是“核心 harness”
- 某套技能系统更适合放在 `harness engineering` 的实践层，而不是直接当成 `agent harness`
- 某篇文章强调的是“长任务 harness 设计”，不是“客户端协议”
...
文中用它来指人如何设计、调试、约束、简化和演化这层外壳及相关机制的工程实践。它偏方法与实践。
...
它可以与 harness 深度重叠，但不自动等于 harness。尤其当一个系统主要讨论的是阶段流转和任务分工，而不是主工作面、恢复面和控制面时，更适合优先看作工作流 / 编排系统。

### `技能 / 方法系统（skills / methodology）`

它更偏执行纪律和方法层，例如规格驱动开发、评审纪律、上下文工程和检查清单。

它常常落在 `harness engineering` 的实践层里，也可能被某个 harness 吸纳，但不自动等于 `agent harness` 本身。
...
2. 再问：这个判断是在什么语境下成立？
3. 最后问：它是在说 `host`、`agent harness`、`harness engineering` 的实践层，还是 `workflow`？
